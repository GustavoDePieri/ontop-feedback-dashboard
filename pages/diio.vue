  ...
  <div class="min-h-screen bg-gradient-dark p-4 sm:p-6 lg:p-8">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between flex-wrap gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <NuxtLink
              to="/sync"
              class="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-all duration-200"
            >
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
              <span class="hidden sm:inline">Sync Client IDs</span>
            </NuxtLink>
            ...
            >
              ...
              <span class="hidden sm:inline">{{ syncing ? 'Syncing...' : 'Sync New Transcripts' }}</span>
      ...
      <div class="bg-white/5 backdrop-blur-sm rounded-lg p-6 border border-white/10">
        ...
        <!-- Empty State -->
        <div v-else-if="filteredTranscripts.length === 0" class="text-center py-20 fade-in">
          <div class="w-32 h-32 bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6 backdrop-blur-sm border border-white/10 shadow-lg">
            <svg class="w-16 h-16 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
          </div>
          <h3 class="text-white text-2xl font-bold mb-3">{{ transcripts.length === 0 ? 'No Transcripts Found' : 'No Matching Transcripts' }}</h3>
          <p class="text-gray-400 mb-8 text-lg">{{ transcripts.length === 0 ? 'Click "Sync New Transcripts" to fetch transcripts from DIIO' : 'Try adjusting your search criteria' }}</p>
        </div>
            ...
            <div class="flex items-start justify-between">
              ...
              <div class="ml-4 flex flex-col items-end gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <!-- Feedback button - shows if feedback data exists -->
                  <!-- <button
...
const showSentimentAnalysis = (transcript: any) => {
  ...
  aiAnalysisResult.value = {
    analysis: {
      ...
      // NEW FIELDS from BART
      sentimentExplanation: sentimentData.sentimentExplanation || '',
      keywords: sentimentData.keywords || [],
      keyQuotes: sentimentData.keyQuotes || { positive: [], negative: [], neutral: [] },
      enhancedWithAI: sentimentData.enhancedWithAI || false
    },
    metadata: {

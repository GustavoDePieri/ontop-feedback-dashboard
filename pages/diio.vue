    ...
    <div class="max-w-7xl mx-auto">
      ...
      >
        <div class="bg-gray-800 rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-auto border border-gray-700 shadow-2xl">
          ...
          <!-- Sentiment Explanation (NEW!) -->
          <div v-if="aiAnalysisResult.analysis.sentimentExplanation" class="mb-6 p-4 bg-gradient-to-r from-purple-900/30 to-pink-900/30 rounded-lg border border-purple-500/30">
            ...
            <p class="text-white text-sm leading-relaxed">{{ aiAnalysisResult.analysis.sentimentExplanation }}</p>
          </div>
          ...
          <!-- Keywords (NEW!) -->
          <div v-if="aiAnalysisResult.analysis.keywords && aiAnalysisResult.analysis.keywords.length > 0" class="mb-6">
            ...
            <div class="flex flex-wrap gap-2">
              <span v-for="(keyword, idx) in aiAnalysisResult.analysis.keywords" :key="idx"
                class="px-3 py-1 text-sm rounded-full bg-gray-700 text-gray-200 border border-gray-600">
                {{ keyword }}
              </span>
            </div>
          </div>
          ...
          <div v-if="aiAnalysisResult.analysis.painPoints && aiAnalysisResult.analysis.painPoints.length > 0" class="mb-6">
            <h3 class="text-red-400 font-semibold mb-3 flex items-center gap-2">
              ...
              Concerning Quotes from Meeting
            </h3>
            <div class="space-y-2">
              <div v-for="(point, idx) in aiAnalysisResult.analysis.painPoints" :key="idx" 
                ...
                <span class="text-red-300 text-sm italic">"{{ point }}"</span>
              </div>
            </div>
          </div>
          ...
          <div v-if="aiAnalysisResult.analysis.positiveHighlights && aiAnalysisResult.analysis.positiveHighlights.length > 0" class="mb-6">
            ...
            <div class="space-y-2">
              <div v-for="(highlight, idx) in aiAnalysisResult.analysis.positiveHighlights" :key="idx" 
                ...
                <span class="text-green-300 text-sm italic">"{{ highlight }}"</span>
              </div>
            </div>
          </div>
